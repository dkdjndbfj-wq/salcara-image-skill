...
为避免图片模型把长文字生成成乱码、错别字或看似文字的伪字符，制作海报、菜单、价格表、信息图、活动日程等图片时，会先区分两种文字：短标题、Logo 字样和装饰性标语属于画面组成，可以要求生图模型生成艺术字；正文、价格、日期、联系方式、表格等必须准确的信息文字，则留到生成后排版。

Agent 会预先规划艺术字区域、正文区域、层级、留白和安全边距。生图提示词可以包含用户指定的艺术字，但会禁止模型额外生成杂字、伪文字和水印，并为信息文字预留干净区域。图片返回后，再使用 SVG、HTML/Canvas、Sharp、ImageMagick 等本地确定性方式叠加准确文字，检查错别字、换行、对齐、对比度和裁切。后续本地排版不调用 Salcara 生图接口；能通过本地修正的问题不会重新付费生成。
